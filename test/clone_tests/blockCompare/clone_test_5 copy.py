...
def jpeg_style_compression(image, wavelet='haar', level=1, scale=(8, 8)):
...
def run():
    ...
    for x in range(0, w, frameSize):
        for y in range(0, h, frameSize):
            if x + frameSize > w or y + frameSize > h:
                ...
                compressed = jpeg_style_compression(compare_img, scale=(8, 8), level=3)
                    ...
                    if hashbundle[0] - phash < 20:
                        ...
                        if len(matches) > 200:
                            for match in matches:
                                ...
                                areaA = (toReal(match[0][0]), toReal(match[0][1]), toReal(match[0][0]) + frameSize * scale, toReal(match[0][1]) + frameSize * scale)
    ...
    quant_image.show()
    out.show()
    out.save("C:\\Users\\Kapsr\\Pictures\\test_image_folder\\klon_bearbeitet_result_5.png")    
